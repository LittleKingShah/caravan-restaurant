    ...
    >
      <Helmet>
        ...
        {SUPPORTED_LANGS.map((code) => (
          <link key={code} rel="alternate" hrefLang={code} href={`${SITE_URL}/${code}/menu`} />
        ))}
        <link rel="alternate" hrefLang="x-default" href={`${SITE_URL}/en/menu`} />
      ...
      <section className="py-16 sm:py-24 bg-caravan-dark relative">
        ...
        <div className="relative max-w-7xl mx-auto px-6 sm:px-8 lg:px-12">
          {/* Category tabs */}
          <div className="sticky top-20 lg:top-24 z-30 bg-caravan-dark/95 pb-8 pt-4 -mt-4">
            <div className="flex flex-wrap justify-center gap-2 sm:gap-3" role="tablist" aria-label={t('menu.categoryLabel')}>
              {categories.map((cat) => (
                <button
                  key={cat}
                  role="tab"
                  ...
                  className={`relative px-5 py-2.5 text-[11px] uppercase tracking-[0.15em] font-medium transition-colors duration-200 cursor-pointer focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-caravan-gold ${
                    activeCategory === cat
          ...
          {activeCategory !== 'All' && (
            <SectionHeading title={t(`categories.${activeCategory}`, { defaultValue: activeCategory })} />
          )}

          {/* Grid */}
          <motion.div layout role="tabpanel" aria-label={t('menu.panelLabel', { category: t(`categories.${activeCategory}`, { defaultValue: activeCategory }) })} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
